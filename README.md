...
DSH Windows Launcher 是面向 Windows 的 **DSH 配对集中端独立客户端**：它集中管理网络可达的
DeepSeek Harness 实例（支持局域网、公网域名、公网 IPv4/IPv6）的「DSH 远程访问」配对，持续
发送心跳保活配对连接，并在客户端内内嵌显示 Harness 远程界面。它不安装、不启动、不管理远端
Harness 进程，管理操作全程无需外部浏览器。

## 产品形态

- 独立客户端窗口（WPF 原生）：目标列表（配对/连接/心跳/异常），粘贴配对链接添加
  目标、重新配对、重命名、删除、手动心跳、保活开关；点击关闭按钮时询问「退出程序」
  或「最小化到托盘」，可勾选「记住我的选择」，也可在托盘菜单「关闭窗口时」随时改回；
  最小化后保活继续。整体为极简科技风深色界面（深蓝黑底 + 青色强调）。
- 内嵌远程界面：每个目标可在客户端内打开一个 WebView2 窗口，直接显示 Harness 的
  `pair-app?device=<id>` 远程界面（按目标隔离的用户数据目录）；弹窗折叠回当前视图，
  全程不调用外部浏览器。窗口左侧提供可折叠的目标侧边栏，展示每个目标的域名/IP、
  配对与连接状态（心跳实时刷新），点击其他目标可直接打开其远程窗口。
- 托盘常驻：单实例运行，托盘图标展示实时配对计数，双击重新打开管理窗口，菜单提供
  打开管理窗口、关闭窗口时（询问/最小化/退出，带勾选标记）与退出。
- 公网连接兼容：配对、状态查询和心跳使用内置 WebView2 网络组件，改善浏览器可访问但
  Windows 系统 TLS 握手失败的连接兼容性，继续校验 HTTPS 证书并隔离每个目标的凭据。
...
1. 在 Harness 桌面端「远程访问」面板铸造二维码并复制配对链接
   （`http(s)://<域名或IP>[:<port>]/pair-accept?pair=<token>`）。
...
- 固定域名中继：`https://<id>.dsh-market.com/pair-accept?pair=<token>`。
- Cloudflare 快速隧道：`https://<随机域名>.trycloudflare.com/pair-accept?pair=<token>`。
- 命名隧道或手动公网地址：`https://<自有域名>/pair-accept?pair=<token>`。

在插件设置中开启自动公网隧道，待远程访问面板显示公网链接后，将完整链接粘贴到「添加目标」。
使用隧道时无需向公网开放 Harness 监听端口，也无需开启局域网绑定。固定域名中继在主机重启后
保持地址不变；中继暂时离线时客户端保留配对，持续退避重试，恢复后自动继续保活。
关闭固定域名中继时，快速隧道地址可能随主机重启变化；请复制新链接添加目标，再删除旧地址目标。

直接使用公网 IP 或域名时，需要配置 NAT、安全组和防火墙，让 Harness 服务端口可达。
公网部署建议使用有效证书的 HTTPS；客户端不会跳过 TLS 证书校验。HTTP 端点可以在同一域名内
升级到 HTTPS，配对请求和心跳会保留原始方法与凭据，不会跟随到其他域名。

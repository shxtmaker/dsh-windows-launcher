...
**品牌图标**：
由原创 mascot 位图生成的应用图标，按尺寸分两档：大尺寸用整幅构图，小尺寸改用脸部放大裁切。
它不得引用上游官方标志或微软徽标图形；产物由 `eng/make-app-icon.ps1` 可复现生成，不手工修图。
_Avoid_: 上游官方标志、微软四格旗帜、构建时临时生成的图标、单一构图直接降采样到小尺寸

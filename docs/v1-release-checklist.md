...
| 正式产物 | 按用户 EXE 安装包，内含 apphost EXE 与托管主程序集，.NET 10 自包含 `win-x64`；Authenticode 可选但状态必须显式记录 |
...
精确 .NET SDK、Inno Setup、最低 Evergreen WebView2 Runtime、Publisher、签名策略和发布地址以候选版本发布常量为准。策略为 `required` 时证书主体和 HTTPS 时间戳服务也必须冻结。任一策略必需值缺失或仍是占位符时，不得形成正式候选版本。
...
2. 按发布常量运行 `eng/package.ps1`（强制签名）或 `eng/package-unsigned.ps1`（可选签名策略），冻结唯一安装包并计算 SHA-256。
3. 对该精确安装包运行 `eng/release-smoke.ps1`，完成下表中适用的实机冒烟，生成 `release-evidence.md`，再进行一次人工发布确认。

第 2 步必须先完成策略要求的签名处理，再冻结最终安装包 SHA-256。冻结后不得重新打包、改写资源或修改任何字节。未签名正式包必须在 manifest、校验和和 Release 正文中明确记录 `NotSigned`。
...
- 唯一候选安装包及其 SHA-256；
- 安装器、apphost EXE、托管主程序集和卸载器的冻结哈希及实测 Authenticode 状态；策略为 `required` 时还须有效签名和可信时间戳；
...
| `VFY-08` 供应链、注册表与证据 | 来源和版本锁、冻结发布应用集、实测 Authenticode 状态、输入二进制哈希、许可证、SBOM、漏洞、秘密扫描；统一 JCS 字节规范化、注册表 Schema/版本/冲突/墓碑/摘要、治理状态机、生成一致性、影响清单、支持记录、证据身份、结果模式和敏感字段扫描。 |
...
|---|---|---|---|
| `RS-01` | 每次；Windows 11 VM | 校验签名策略已执行并记录实测状态，再冻结文件名、大小、版本和 SHA-256；核对 Publisher、SBOM 和依赖基线。安装包、apphost EXE、托管主程序集、卸载器和嵌入注册表摘要身份一致，此后不得改变任何字节。 | `REQ-DIST-001`、`REQ-DIST-002` |
...
| `RS-14` | 每次含普通卸载；升级/修复部分在首版或安装器/数据模式/回退水位变化时执行；Windows 11 VM | 验证同版修复、从固定的前一 RC 到当前候选的就地升级、拒绝降级、运行中请求正常退出、已知数据迁移及未知模式失效保护。验证注册表水位单调提高、低版本/未知新 Schema 只保留基础兼容、忘记目标精确删除确认与诊断缓存且不降低水位。普通卸载保留数据；数据清除卸载只删精确应用数据根且不移除共享 Runtime。 | `REQ-DATA-006`、`REQ-DATA-007`、`REQ-DIST-007` 至 `REQ-DIST-017` |
...
| 安装器、发布常量、签名策略、.NET 自包含 Runtime、WebView2 前置 | `RS-03`、`RS-14`、`RS-15` 及完整安装生命周期数据集。 |
...
- Installer size:
- Installer SHA-256:
- Installer Authenticode status, signer subject and timestamp when present:
...
- 候选安装包哈希与报告不一致；
- 契约、Schema、注册表、规则/墓碑、支持记录、影响清单、页面能力快照或参考适配器身份与冻结候选不一致；
...
- Authenticode 状态、Publisher、版本或发布常量缺失或不一致；策略为 `required` 时签名或可信时间戳缺失；

...
| 插件候选 tarball | `plugins/dsh-remote-attachments/pack/shxtmaker-dsh-remote-attachments-0.1.0.tgz` | `sha256=a6f865a439297307501a58c9491e75aafb4438ad05c9f58a723a84923b55aac1`，59 文件，165 802 B |
...
- 仓库纪律提醒：`compatibility-lock.json` 的插件 tarball 摘要是 **D03 轮次快照**（`5d4016be…`），不随构建更新；**Windows 侧引用插件包摘要一律以本清单 `tarball.sha256`（`a6f865a4…`）为准**。
...
Get-FileHash plugins/dsh-remote-attachments/pack/shxtmaker-dsh-remote-attachments-0.1.0.tgz -Algorithm SHA256  # 期望 a6f865a4…
...
pwsh -File .\eng\package.ps1 -Version 2.1.1 -PackageMode Offline ... -AllowInstallerExecutionForUninstallerVerification
pwsh -File .\eng\package.ps1 -Version 2.1.1 -PackageMode Online  ... -AllowInstallerExecutionForUninstallerVerification
```
